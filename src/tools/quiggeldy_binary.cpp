...
void configure(WorkerT& worker, Config& cfg)
{
	...
	if (cfg.no_munge) {
		...
		HXCOMM_LOG_DEBUG(log, "Enabling munge authentication.");
		worker.set_use_munge(true);
	}
}
...
int main(int argc, const char* argv[])
{
	...
	("connect-ip,i", po::value<std::string>()->default_value(""),
	 "IP to connect to (will be inferred from environment if available)")
