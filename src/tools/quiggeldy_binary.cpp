...
int main(int argc, const char* argv[])
{
	...
	if (timeout_reached) {
		...
		// cancel sighandler only if the shutdown was not initiated via signal
		RCF_LOG_TRACE(log, "Cancelling signal handler.")
		if (pthread_cancel(sig_thread) != 0) {
			RCF_LOG_FATAL(log, "Could not cancel signal handling thread.");
		}
	}

	if (pthread_join(sig_thread, NULL) != 0) {
		RCF_LOG_FATAL(log, "Could not join signal handling thread.");
	}
	RCF::deinit();
	if (work_left_at_shutdown) {
		RCF_LOG_ERROR(log, "There was work left on scheduler shutdown.");
		return 1;
	} else {
		return 0;
	}
