...
pid_t launch_with_allocated_slurm(std::vector<std::string> const& arg_vec, std::tuple<Args...> base_args)
{
	if (std::getenv(vision_slurm_fpga_ips_env_name) != nullptr) {
		...
		pid = std::apply(
		    [](auto... args) -> pid_t { return hxcomm::setup_quiggeldy_with_argv(arg_vec, args...); }, conn_args);
	} else {
...
std::tuple<pid_t, hxcomm::port_t> launch_quiggeldy_locally_from_env()
{
	...
	auto base_args = std::make_tuple(
	    "quiggeldy", port, "--no-allocate-license", "--timeout", "10", "--release", "86400",
	    hxcomm::is_munge_available() ? "" : "--no-munge");

	pid_t pid = launch_with_allocated_slurm(std::vector<std::string>{}, base_args);
...
std::tuple<pid_t, hxcomm::port_t> launch_quiggeldy_locally_from_env_argv(std::vector<std::string> const& argv)
{
    auto const port = get_unused_port();

    auto base_args = std::make_tuple(
        "quiggeldy", port, hxcomm::is_munge_available() ? "" : "--no-munge");
