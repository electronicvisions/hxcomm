...
template <typename ConnectionParameter, typename RcfClient>
void QuiggeldyConnection<ConnectionParameter, RcfClient>::register_reinit(
    typename interface_types::reinit_type&& reinit)
{
	m_uploader_reinit->upload(std::move(reinit));
}

template <typename ConnectionParameter, typename RcfClient>
void QuiggeldyConnection<ConnectionParameter, RcfClient>::register_reinit(
    std::shared_ptr<typename interface_types::reinit_type> const& reinit)
{
	m_uploader_reinit->upload(reinit);
}

template <typename ConnectionParameter, typename RcfClient>
std::weak_ptr<typename QuiggeldyConnection<ConnectionParameter, RcfClient>::uploader_reinit_type>
QuiggeldyConnection<ConnectionParameter, RcfClient>::get_uploader_reinit() const
{
	return m_uploader_reinit;
}
